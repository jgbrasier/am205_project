...
# noisify data
noise = 1.0*np.random.randn(U.shape[0], U.shape[1])
U_noise = U + noise
U_noise_norm = U_noise/np.max(U_noise)
plt.figure()
...
train(model, X, y, optimizer,sparsity_scheduler, log_dir='runs/2DAD/', split=0.8, max_iterations=100000, delta=1e-4, patience=8) 
